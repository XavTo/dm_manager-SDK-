...
bool CheckError(char string[256], Handle handle, bool close)
{
    ...
    if (handle == null) {
        ...
        PrintToServer("[DM_MANAGER]No error when %s", string);
        if (close)
            CloseHandle(handle);
        return true;
    }
...
void setUsersStorage(int clientId, int idUser, char steamId[32])
{
    ...
    if (weaponInfo.kills >= 100 && (float(weaponInfo.hs) / float(weaponInfo.kills) * 100) >= 50)
        StrCat(buffer, sizeof(buffer), "-Global");
    PrintToServer("[DM_MANAGER]Weapon: %s", buffer);
    if (!KvJumpToKey(kv, buffer)) {
        PrintToServer("[DM_MANAGER]No weapon found in kv");
        KvRewind(kv);
        return;
    }
    usersStorage[i].rankIdPic = KvGetNum(kv, "index");
    KvRewind(kv);
}
